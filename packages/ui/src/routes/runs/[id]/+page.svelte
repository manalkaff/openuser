<script lang="ts">
  ...
  function verdictColor(verdict: string | null): string {
    if (verdict === 'goal_achieved') return 'text-success';
    if (verdict === 'blocked') return 'text-warning';
...
<div class="p-8 min-h-full">
  {#if loading}
    <div class="space-y-4">
      <div class="animate-pulse rounded-xl ring-1 ring-foreground/10 bg-card h-24"></div>
      <div class="animate-pulse rounded-xl ring-1 ring-foreground/10 bg-card h-96"></div>
    </div>
  {:else if error}
  ...
  {:else if run}
    ...
    {#if effectiveStatus === 'passed' || effectiveStatus === 'failed' || effectiveStatus === 'blocked' || effectiveStatus === 'aborted'}
      <div class="mb-6 rounded-xl ring-1 {effectiveStatus === 'passed' ? 'ring-success/30 bg-success/5' : 'ring-foreground/10 bg-card'} p-5">
          ...
          <div class="flex items-center gap-3">
            {#if run.testId === null}
              <button
                ...
                class="rounded-lg border border-brand/40 bg-brand/10 px-4 py-1.5 text-sm font-medium text-brand hover:bg-brand/20 transition-colors"
              >
                Promote to test
              </button>
            {/if}
            <RelativeTime timestamp={run.finishedAt ?? run.startedAt} class="text-xs text-muted-foreground" />
          </div>
        </div>
        {#if run.summary}
          <p class="mt-3 text-sm text-foreground/80 border-t border-border pt-3">{run.summary}</p>
        {/if}
      </div>
    {:else if effectiveStatus === 'running'}
      <div class="mb-6 rounded-xl ring-1 ring-info/30 bg-info/5 p-4 flex items-center gap-3">
        <span class="h-2 w-2 rounded-full bg-info animate-pulse"></span>
        <span class="text-sm text-info font-medium">Run in progress — watching live…</span>
        <RelativeTime timestamp={run.startedAt} class="ml-auto text-xs text-muted-foreground" />
      </div>
    {/if}
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <Tabs tabs={tabDefs} active={activeTab} onchange={(id) => { activeTab = id; }}>
          {#snippet children()}
              ...
              <div class="space-y-2">
                ...
                {:else}
                  {#each allSteps as step (step.id)}
                    <div class="flex gap-3 rounded-lg ring-1 {step.status === 'error' ? 'ring-destructive/30 bg-destructive/5' : 'ring-foreground/10 bg-card'} p-3">
                      <div class="flex flex-col items-center">
                        <span class="text-lg" aria-hidden="true">{stepKindIcon(step.kind)}</span>
                        <span class="text-xs text-muted-foreground/40 mt-1">#{step.idx}</span>
                      </div>
                      <div class="flex-1 min-w-0">
                        <div class="flex items-start justify-between gap-2">
                          <div>
                            <span class="text-xs font-mono text-muted-foreground uppercase">{step.kind}</span>
                            <p class="text-sm text-foreground mt-0.5">{step.description}</p>
                            {#if step.pageUrl}
                              <p class="text-xs text-muted-foreground font-mono truncate mt-0.5">{step.pageUrl}</p>
                            {/if}
                            {#if step.note}
                              <p class="mt-1.5 text-xs italic text-muted-foreground border-l-2 border-border pl-2">"{step.note}"</p>
                            {/if}
                            {#if step.error}
                              <p class="mt-1.5 text-xs text-destructive font-mono">{step.error}</p>
                            {/if}
                          </div>
                          <div class="flex items-center gap-2 shrink-0">
                            {#if step.durationMs !== null && step.durationMs !== undefined}
                              <span class="text-xs text-muted-foreground/40">{step.durationMs}ms</span>
                            {/if}
                  ...
                  {#if effectiveStatus === 'running'}
                    <div class="flex items-center gap-2 py-2 pl-4 text-xs text-muted-foreground">
            ...
            {:else if activeTab === 'screenshots'}
              ...
              {#if shots.length === 0}
                <p class="text-sm text-muted-foreground py-4 text-center">No screenshots yet.</p>
              {:else}
                <div class="grid grid-cols-2 gap-3 sm:grid-cols-3">
                  {#each shots as step (step.id)}
                    <button
                      type="button"
                      onclick={() => { enlargedShot = `/artifacts/${runId}/${step.screenshotPath}`; }}
                      class="group relative aspect-video overflow-hidden rounded-lg ring-1 ring-foreground/10 bg-card hover:ring-foreground/20 transition-all"
                    >
                      <img
                      ...
                      <div class="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-colors flex items-end">
                        <p class="w-full bg-black/60 px-2 py-1 text-xs text-foreground truncate opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                {#if run!.videoPath}
                  <div class="mt-6">
                    <h3 class="text-sm font-medium text-foreground mb-2">Run recording</h3>
                    <video
                      controls
                      class="w-full rounded-lg ring-1 ring-foreground/10 bg-black max-h-96"
            ...
            {:else if activeTab === 'console'}
              {#if consoleEvents.length === 0}
                <p class="text-sm text-muted-foreground py-4 text-center">No error-level console events.</p>
              {:else}
                <div class="rounded-lg ring-1 ring-foreground/10 bg-background font-mono text-xs overflow-auto max-h-[50vh]">
                  {#each consoleEvents as evt (evt.id)}
                    {@const cp = evt.payload as { text?: string }}
                    <div class="flex gap-2 px-3 py-1.5 border-b border-border last:border-0 text-destructive">
                      <span class="text-muted-foreground/40 shrink-0">#{evt.stepIdx}</span>
                      <span class="text-muted-foreground shrink-0 uppercase text-[10px] leading-4 w-12">{evt.level}</span>
            ...
            {:else if activeTab === 'network'}
              {#if networkEvents.length === 0}
                <p class="text-sm text-muted-foreground py-4 text-center">No failed network requests.</p>
              {:else}
                <div class="rounded-xl ring-1 ring-foreground/10 bg-card overflow-hidden">
                  <table class="w-full text-xs font-mono">
                    <thead>
                      <tr class="border-b border-border text-muted-foreground uppercase text-[10px] tracking-wider">
                    ...
                    <tbody>
                      {#each networkEvents as evt (evt.id)}
                        ...
                        <tr class="border-b border-border last:border-0 hover:bg-accent/50">
                          <td class="py-2 px-3 text-muted-foreground">#{evt.stepIdx}</td>
                          <td class="py-2 px-3 text-destructive">{p.status ?? evt.level}</td>
                          <td class="py-2 px-3 text-muted-foreground">{p.method ?? '—'}</td>
                          <td class="py-2 px-3 text-foreground truncate max-w-xs" title={p.url}>{p.url ?? '—'}</td>
                        </tr>
                      {/each}
                    </tbody>
                  </table>
            ...
            {:else if activeTab === 'findings'}
              {#if run!.findings.length === 0}
                <p class="text-sm text-muted-foreground py-4 text-center">No findings for this run.</p>
              {:else}
                <div class="space-y-3">
                  {#each run!.findings as finding (finding.id)}
                    <div class="rounded-lg ring-1 ring-foreground/10 bg-card p-4">
                      <div class="flex items-start justify-between gap-3 mb-2">
                        <div class="flex items-center gap-2 flex-wrap">
                          <SeverityBadge severity={finding.severity} />
                          <span class="text-xs text-muted-foreground bg-muted px-2 py-0.5 rounded capitalize">{finding.type.replace('_', ' ')}</span>
                        </div>
                        <span class="text-xs text-muted-foreground capitalize">{finding.status}</span>
                      </div>
                      <h4 class="text-sm font-medium text-foreground">{finding.title}</h4>
                      <p class="mt-1 text-sm text-muted-foreground italic">"{finding.description}"</p>
                      {#if finding.evidence.screenshotPath}
                        <button
                          type="button"
                          onclick={() => { enlargedShot = `/artifacts/${runId}/${finding.evidence.screenshotPath}`; }}
                          class="mt-2 text-xs text-brand hover:text-brand/80 transition-colors"
                        >
                          View screenshot →
                        </button>
                      {/if}
      ...
      <div class="space-y-4">
        <!-- Run meta -->
        <div class="rounded-xl ring-1 ring-foreground/10 bg-card p-4 text-sm space-y-2">
          <h3 class="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-3">Run info</h3>
          ...
          {#if run.environmentName}
            <div class="flex justify-between gap-2">
              ...
              <span class="text-foreground">{run.environmentName}</span>
            </div>
          {/if}
          <div class="flex justify-between gap-2">
            <span class="text-muted-foreground">Base URL</span>
            <span class="text-foreground font-mono text-xs truncate max-w-[10rem]" title={run.baseUrlResolved}>
              {run.baseUrlResolved}
            </span>
          </div>
          {#if run.agentLabel}
            <div class="flex justify-between gap-2">
              ...
              <span class="text-foreground">{run.agentLabel}</span>
            </div>
          {/if}
          <div class="flex justify-between gap-2">
            <span class="text-muted-foreground">Started</span>
            <RelativeTime timestamp={run.startedAt} class="text-foreground" />
          </div>
          {#if run.finishedAt}
            <div class="flex justify-between gap-2">
              <span class="text-muted-foreground">Finished</span>
              <RelativeTime timestamp={run.finishedAt} class="text-foreground" />
            </div>
          {/if}
          {#if run.testId === null && run.adhocGoal}
            <div class="pt-2 border-t border-border">
              <p class="text-xs text-muted-foreground mb-1">Ad-hoc goal</p>
              <p class="text-xs text-foreground">{run.adhocGoal}</p>
            </div>
          {/if}
        </div>
        ...
        {#if persona}
          <PersonaCard {persona} />
        {:else}
          <div class="rounded-xl ring-1 ring-foreground/10 bg-card p-4 text-xs text-muted-foreground">
...
{#if enlargedShot}
  ...
  >
    ...
    <button
      type="button"
      onclick={() => { enlargedShot = null; }}
      class="absolute top-4 right-4 rounded-full bg-card p-2 text-muted-foreground hover:text-foreground"
      aria-label="Close"
    >
...
>
  {#snippet children()}
    <div class="space-y-4">
      <p class="text-sm text-muted-foreground">Save this ad-hoc run as a named test that can be run again with any persona.</p>
      <div>
        <label class="block text-sm font-medium text-foreground mb-1.5" for="promote-title">Test title *</label>
        <input
          id="promote-title"
      ...
      <div>
        <label class="block text-sm font-medium text-foreground mb-1.5" for="promote-priority">Priority</label>
        <select
          id="promote-priority"
      ...
      <div>
        <label class="block text-sm font-medium text-foreground mb-1.5" for="promote-tags">Tags</label>
        <input
          id="promote-tags"
